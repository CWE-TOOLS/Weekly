-- =====================================================================
-- Import casting inventory for Project 0383, Phase 2, Cast #4
...
-- Existing inventory under Cast #4 (Phase 2) is wiped first so re-running is safe.
-- Lookup is scoped to phase_id so Cast #4 from another phase won't collide.
-- =====================================================================
...
BEGIN
    ...
    -- because Cast #4 also exists under Phase 1 on this project.
    SELECT count(*) INTO v_count
    FROM project_castings
    WHERE project_number = '0383'
      AND casting_number = '4'
      AND phase_id = 'ef66ab51-0e47-4639-8297-9e0bd0ed7768';

    IF v_count = 0 THEN
        RAISE EXCEPTION 'No casting #4 found for project 0383 under Phase 2';
    ELSIF v_count > 1 THEN
        RAISE EXCEPTION 'Multiple casting #4 rows found for project 0383 under Phase 2 (% rows).', v_count;
    END IF;
