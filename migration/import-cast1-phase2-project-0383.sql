-- =====================================================================
-- Import casting inventory for Project 0383, Phase 2, Cast #1
-- Source: panel inventory list provided by user (length, width, qty per type)
--
-- 12 type/dimension rows · 81 panels total.
--
-- Color and sealer are left blank — set them in the Casting Inventory UI if needed.
-- Existing inventory under Cast #1 is wiped first so re-running is safe.
-- The DO block fails fast if Cast #1 is missing or ambiguous (multiple
...
BEGIN
    ...
    -- because Cast #1 also exists under Phase 1 on this project.
    SELECT count(*) INTO v_count
    FROM project_castings
    WHERE project_number = '0383'
      AND casting_number = '1'
      AND phase_id = 'ef66ab51-0e47-4639-8297-9e0bd0ed7768';

    IF v_count = 0 THEN
        RAISE EXCEPTION 'No casting #1 found for project 0383 under Phase 2';
    ELSIF v_count > 1 THEN
        RAISE EXCEPTION 'Multiple casting #1 rows found for project 0383 under Phase 2 (% rows).', v_count;
    END IF;
    ...
    INSERT INTO casting_inventory (casting_id, type, width, length, quantity, extras, sort_order) VALUES
        ...
        (v_casting_id, '144-Z', '8"',      '144"',  1, 0, 11);

    RAISE NOTICE 'Imported 12 inventory rows (81 panels) into casting %', v_casting_id;
END $$;
