-- =====================================================================
-- Import casting inventory for Project 0383, Phase 2, Cast #2
...
-- Existing inventory under Cast #2 is wiped first so re-running is safe.
-- The DO block fails fast if Cast #2 is missing or ambiguous (multiple
...
BEGIN
    ...
    -- because Cast #2 also exists under Phase 1 on this project.
    SELECT count(*) INTO v_count
    FROM project_castings
    WHERE project_number = '0383'
      AND casting_number = '2'
      AND phase_id = 'ef66ab51-0e47-4639-8297-9e0bd0ed7768';

    IF v_count = 0 THEN
        RAISE EXCEPTION 'No casting #2 found for project 0383 under Phase 2';
    ELSIF v_count > 1 THEN
        RAISE EXCEPTION 'Multiple casting #2 rows found for project 0383 under Phase 2 (% rows).', v_count;
    END IF;
