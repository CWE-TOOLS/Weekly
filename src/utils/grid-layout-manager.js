...
/**
 * Calculate week index from scroll position
 * @param {number} scrollPosition - Current scroll position
 * @param {number} wrapperWidth - Width of the wrapper element
 * @returns {number} Calculated week index
 */
export function calculateWeekIndexFromScroll(scrollPosition, wrapperWidth) {
    return Math.round(scrollPosition / wrapperWidth);
}

/**
 * Preserve scroll position during re-render
 * @param {HTMLElement} wrapper - Schedule wrapper element
 * @param {number} previousScrollPosition - Previous scroll position
 * @param {number} wrapperWidth - Width of wrapper element
 * @returns {Object} Object with scrollLeft and weekIndex
 */
export function preserveScrollPosition(wrapper, previousScrollPosition, wrapperWidth) {
    if (previousScrollPosition > 0 && previousScrollPosition <= wrapper.scrollWidth) {
        const weekIndex = calculateWeekIndexFromScroll(previousScrollPosition, wrapperWidth);
        return { scrollLeft: previousScrollPosition, weekIndex };
    }
    return null;
}

/**
 * Restore saved scroll position from storage
 * @param {HTMLElement} wrapper - Schedule wrapper element
 * @param {number} savedScrollPosition - Saved scroll position
 * @param {number} wrapperWidth - Width of wrapper element
 * @returns {Object|null} Object with scrollLeft and weekIndex, or null if invalid
 */
export function restoreScrollPosition(wrapper, savedScrollPosition, wrapperWidth) {
    if (savedScrollPosition !== null && savedScrollPosition >= 0 && savedScrollPosition <= wrapper.scrollWidth) {
        const weekIndex = calculateWeekIndexFromScroll(savedScrollPosition, wrapperWidth);
        return { scrollLeft: savedScrollPosition, weekIndex };
    }
    return null;
}
