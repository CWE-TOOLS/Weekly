-- ============================================================================
-- Fix Supabase Cache Functions (v3)
-- Fixes variable name conflict with PostgreSQL built-in current_time
-- ============================================================================
...
-- 2. Create acquire_cache_update_lock - FIXED variable name
-- ============================================================================
...
DECLARE
  ...
  now_ts TIMESTAMPTZ := NOW();  -- Changed from current_time to now_ts
...
BEGIN
  ...
  SELECT
    ...
    (update_started_at IS NULL OR
     (update_started_at + make_interval(secs => lock_timeout_seconds)) < now_ts) AS is_expired
...
SELECT acquire_cache_update_lock('test-client-v3', 'tasks') as lock_acquired;
...
-- ============================================================================
-- Fix Complete - This should work now!
-- ============================================================================
-- The issue was using 'current_time' which is a PostgreSQL built-in function
-- Now using 'now_ts' which doesn't conflict
-- ============================================================================
